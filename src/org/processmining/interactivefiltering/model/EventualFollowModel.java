...
public class EventualFollowModel {
	...
	//Map
	Map<String, Integer> eventualFollowMap;
	...
	public EventualFollowModel(PluginContext context, XLog inputLog) {
		...
		//Map
		eventualFollowMap = new HashMap<String, Integer>();
		...
		for(XTrace trace : inputLog) {
			...
			for(int i = 0; i < allEventList.size(); i++) {
				for(int j = i + 1; j < allEventList.size(); j++) {

					String fromString = getEventName(allEventList.get(i));
					String toString = getEventName(allEventList.get(j));

					//Adding EventualRelation to "relationList"
					String keyString = fromString + "->" + toString; 
					relationList.add(keyString);

					//Adding Map to count frequency
					if(eventualFollowMap.containsKey(keyString)) {
						eventualFollowMap.put(keyString, eventualFollowMap.get(keyString) + 1);
					} else {
						eventualFollowMap.put(keyString, 1);
					}

					//Adding inputAcitivtyMap
					...
					if(eventDataMap.containsKey(keyString)) { //Update 
						...
						eventInfoList.add(allEventList.get(j)); // adding target event inforamtion
						...
						eventInfoList.add(allEventList.get(j));
						eventDataMap.put(keyString, eventInfoList);
						//isSelected information
					...
					if(i == 0) {
						...
						if(eventualFollowMap.containsKey(keyStrForIdx1)) {
							...
							eventualFollowMap.put(keyStrForIdx1, 1);
						}
						...
						//Adding XEvent data to eventDatamap and isSelected Data as well
						if(eventDataMap.containsKey(keyStrForIdx1)) { //Update 
							//XEvent information
							ArrayList<XEvent> eventInfoList = eventDataMap.get(keyStrForIdx1);
							eventInfoList.add(allEventList.get(j)); // adding target event inforamtion
							eventDataMap.put(keyStrForIdx1, eventInfoList);

							//isSelected information
							ArrayList<Boolean> isSelectedList = isSelectedDataMap.get(keyStrForIdx1);
							isSelectedList.add(false);
							isSelectedDataMap.put(keyStrForIdx1, isSelectedList);
						} else { //Init
							//XEvent information
							ArrayList<XEvent> eventInfoList = new ArrayList<XEvent>();
							eventInfoList.add(allEventList.get(j));
						...
						//Adding absFreq
						if(absFreq.containsKey(keyStrForIdx1)) {
							absFreq.put(keyStrForIdx1, absFreq.get(keyStrForIdx1) + 1);
						} else {
					...
					if(j == allEventList.size() - 1) {
						//In case, last event 
						fromString = getEventName(allEventList.get(j));
						toString = "END EVENT (Dummy)";
						String keyStrforLastIdx = fromString + "->" + toString; 
						relationList.add(keyStrforLastIdx);

						//Adding Map to count frequency
						if(eventualFollowMap.containsKey(keyStrforLastIdx)) {
							eventualFollowMap.put(keyStrforLastIdx, eventualFollowMap.get(keyStrforLastIdx) + 1);
						} else {
							eventualFollowMap.put(keyStrforLastIdx, 1);
						}
						...
						//Adding XEvent data to eventDatamap and isSelected Data as well
						if(eventDataMap.containsKey(keyStrforLastIdx)) { //Update 
							//XEvent information
							ArrayList<XEvent> eventInfoList = eventDataMap.get(keyStrforLastIdx);
							eventInfoList.add(allEventList.get(j)); // adding target event inforamtion
							eventDataMap.put(keyStrforLastIdx, eventInfoList);

							//isSelected information
							ArrayList<Boolean> isSelectedList = isSelectedDataMap.get(keyStrforLastIdx);
							isSelectedList.add(false);
							isSelectedDataMap.put(keyStrforLastIdx, isSelectedList);
						} else { //Init
							//XEvent information
							ArrayList<XEvent> eventInfoList = new ArrayList<XEvent>();
							eventInfoList.add(allEventList.get(j));
						...
						//Adding absFreq
						if(absFreq.containsKey(keyStrforLastIdx)) {
							absFreq.put(keyStrforLastIdx, absFreq.get(keyStrforLastIdx) + 1);
						} else {
							absFreq.put(keyStrforLastIdx, 1);
						}
					}
